...
from tensorflow.keras.models import load_model
model = load_model('model111.h5')  # Malaria model = model111.h5
model222=load_model("my_model.h5")  # Pneumonia model 
...
@app.route('/upload', methods=['POST','GET'])  # Predicts malaria or other conditions using the Malaria model
def upload_file():
    ...
    if request.method == 'GET':
        ...
        try:
            ...
            return render_template('predict.html', image_file_name = file.filename, label = label, accuracy = accuracy) ##################### Malaria model
        except:
            flash("Please select the image first !!", "danger")      
            return redirect(url_for("Malaria"))

@app.route('/upload11', methods=['POST','GET']) # Predicts malaria or other conditions using the Pneumonia model
def upload11_file():
    ...
    if request.method == 'GET':
        ...
        try:
            ...
            if(result>50):
                ...
                label= indices[0]
                accuracy= 100-result
            return render_template('predict1.html', image_file_name = file.filename, label = label, accuracy = accuracy) ############### pneumonia  model
        except:
            flash("Please select the image first !!", "danger")      
...
#  Prediction Functions for Different Models
def ValuePredictor(to_predict_list, size):
    to_predict = np.array(to_predict_list).reshape(1,size)
    if(size==8):#Diabetes  (8 features)
        loaded_model = joblib.load("model1")
        result = loaded_model.predict(to_predict)
    elif(size==30):#Cancer (30 features)
        loaded_model = joblib.load("model")
        result = loaded_model.predict(to_predict)
    elif(size==12):#Kidney  (12 features)
        loaded_model = joblib.load("model3")
        result = loaded_model.predict(to_predict)
    elif(size==10):  # liver (10 features)
        loaded_model = joblib.load("model4")
        result = loaded_model.predict(to_predict)
    elif(size==11):#Heart (11 features)
...
def result():
    if request.method == 'POST':
        ...
        elif(len(to_predict_list)==12):#Kidney  (12 features)
            result = ValuePredictor(to_predict_list,12)
        elif(len(to_predict_list)==11):#Heart (11 features)
